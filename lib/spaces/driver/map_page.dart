...
class _MapPageState extends ConsumerState<MapPage> {
  ...
  Future<void> _fetchUserLocation() async {
    try {
      ...
      if (mounted) setState(() { _userLocation = LatLng(pos.latitude, pos.longitude); _locationError = ''; });
    } catch (_) {
      ...
      body: Stack(
        children: [
          FlutterMap(
            ...
            options: MapOptions(initialCenter: _mapCenter, initialZoom: 13.0, maxZoom: 19.0),
            children: [
              if (_mapTheme != null)
                VectorTileLayer(
                  theme: _mapTheme!,
                  ...
                  tileProviders: TileProviders({
                    'openmaptiles': NetworkVectorTileProvider(urlTemplate: _tileServerUrl, maximumZoom: 14),
                  }),
                ),
                ...
                markers: [
                  ...stops.asMap().entries.map((entry) {
                    ...
                    final order = _stopOrders.firstWhere(
                      (o) => (o.deliveryLatitude == point.latitude && o.deliveryLongitude == point.longitude),
                    ...
                    Marker(
                      ...
                      child: Container(
                        ...
                        child: Stack(
                          ...
                          children: [
                            Container(width: 34, height: 34, decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.07), shape: BoxShape.circle)),
                            Container(width: 18, height: 18, decoration: const BoxDecoration(color: Colors.black, shape: BoxShape.circle)),
              ...
              child: widget.activeSubProgram == null
                  ? Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text('No active route assigned', style: TextStyle(fontSize: 14, color: Colors.grey.shade600)),
                      ],
                    )
                  : Row(
                      children: [
                        _bottomStat(Icons.location_on_outlined, '${stops.length}', 'Stops'),
                        _divider(),
                        _bottomStat(Icons.pending_outlined, '$pendingStops', 'Remaining'),
                        _divider(),
                        _bottomStat(Icons.check_circle_outline, '${_stopOrders.where((o) => o.status == 'DELIVERED').length}', 'Delivered'),
                        if (widget.activeSubProgram?.estimatedDistanceKm != null) ...[
                          _divider(),
                          _bottomStat(Icons.straighten_outlined, widget.activeSubProgram!.estimatedDistanceKm!.toStringAsFixed(1), 'km'),
                        ],
                      ],
                    ),
            ),
          ...
          if (_locationError.isNotEmpty)
            Positioned(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(Icons.location_off_outlined, size: 16, color: Colors.grey.shade500),
                    const SizedBox(width: 8),
                    Expanded(child: Text(_locationError, style: TextStyle(fontSize: 12, color: Colors.grey.shade600))),
                  ],
                ),
              ),
            ),
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.location_on_outlined, color: Colors.black87, size: 20),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Stop $stopNumber · ${order.orderNumber}',
                          style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Colors.black87)),
                      Text(order.status, style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
                    ],
                  ),
                ),
              ],
            ...
            const SizedBox(height: 16),
            Text(order.deliveryAddress, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
            if (order.deliveryDescription?.isNotEmpty == true) ...[
              const SizedBox(height: 8),
              Text(order.deliveryDescription!, style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
            ],
            const SizedBox(height: 8),
            Text('${order.weightKg} kg · ${order.volumeM2} m²', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
          ],
        ),
      ),
    );
